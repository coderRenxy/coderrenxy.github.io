...
媒体查询由⼀个可选的媒体类 型和零个或多个使⽤媒体功能的限制了样式表范围的表达式（限制表达式）组成，例如宽度、⾼度和颜⾊。媒体查询，添加⾃ CSS3，允许内容的呈现针对⼀个特定范围的输出设备⽽进⾏裁剪，⽽不必改变内容本身，适合 web ⽹⻚应对不同型号的设备⽽做出对应的响应适配。
...
- 父元素的高度无法被撑开，影响与父元素同级的元素（高度塌陷问题）
- 与浮动元素同级的非浮动元素会跟随其后
...
- 最后一个浮动元素后面添加一个空的div，并设置样式：clear：both
...
层叠顺序，英 文称作 stacking order，表示元素发生层叠时有着特定的垂直显示顺序。下面是盒模型的层叠规则：
